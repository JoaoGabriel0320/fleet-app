...
export default function Dashboard() {
  ...
  // Supabase Realtime — atualiza ao detectar mudanças nas tabelas
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mt-2">
        <h1 className="text-xl font-bold text-slate-800">Visão Geral da Frota</h1>
        {/* Indicador ao vivo */}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-center">
          <Wrench className="w-5 h-5 text-red-600 mx-auto mb-1" />
          <p className="text-2xl font-bold text-red-700">{counts.maintenance}</p>
          <p className="text-xs text-red-600">Manutenção</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Link href="/checkin" className="bg-slate-700 text-white rounded-xl p-3 flex flex-col gap-2 active:bg-slate-800 transition-colors">
      ...
      {upcomingSchedules.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Meus agendamentos</h2>
            <Link href="/schedules" className="text-xs text-blue-700 font-medium">Ver todos</Link>
          </div>
          <div className="space-y-2">
            {upcomingSchedules.map(s => {
              ...
              return (
                <div key={s.id} className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 flex items-start justify-between">
                  <div>
                    <p className="font-semibold text-slate-800 text-sm">{vehicle?.plate} — {vehicle?.brand} {vehicle?.model}</p>
                    {destBranch && <p className="text-xs text-slate-500">→ {destBranch.name}</p>}
                    {s.destination_description && !destBranch && <p className="text-xs text-slate-500">{s.destination_description}</p>}
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-semibold text-indigo-700">{dayLabel}</p>
                    <p className="text-xs text-slate-500">{format(dep, 'HH:mm')}</p>
                  </div>
                </div>
              )
            })}
      ...
      {activeTrips.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Viagens em andamento</h2>
          <div className="space-y-2">
            {activeTrips.map(trip => (
              <div key={trip.id} className="bg-white border border-slate-200 rounded-xl p-3 flex items-start justify-between">
                <div>
                  <p className="font-semibold text-slate-800">
                    {(trip.vehicle as any)?.plate} — {(trip.vehicle as any)?.model}
                  </p>
                  <p className="text-sm text-slate-500">{(trip.collaborator as any)?.name}</p>
                  <p className="text-xs text-slate-400 mt-1">
                    {formatDistanceToNow(new Date(trip.departed_at), { addSuffix: true, locale: ptBR })}
                  </p>
                </div>
                <span className="text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded-full font-medium">em uso</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Vehicles list */}
      <div>
        <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Todos os veículos</h2>
        <div className="space-y-2">
          {vehicles.map(vehicle => (
            <div key={vehicle.id} className="bg-white border border-slate-200 rounded-xl p-3 flex items-center justify-between">
              <div>
                <p className="font-semibold text-slate-800">{vehicle.plate} — {vehicle.brand} {vehicle.model}</p>
