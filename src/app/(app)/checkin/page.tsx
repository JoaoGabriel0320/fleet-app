...
export default function CheckinPage() {
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-4">
        ...
        <h1 className="text-xl font-bold text-slate-800">Devolução registrada!</h1>
        <div className="bg-white border border-slate-200 rounded-xl p-4 w-full max-w-xs space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">KM percorridos</span><span className="font-bold text-blue-700">{summary.kmDriven} km</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Tempo de uso</span><span className="font-semibold">{summary.duration}</span></div>
        </div>
  ...
  if (trips.length === 0 && step === 'select') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-3">
        ...
        <h2 className="text-lg font-semibold text-slate-700">Nenhuma viagem aberta</h2>
        <p className="text-sm text-slate-500">Não há veículos em uso no momento.</p>
        <button onClick={() => router.push('/dashboard')} className="mt-2 border border-slate-300 px-4 py-2 rounded-xl text-sm">Voltar</button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-800 mt-2">Devolução de veículo</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {step === 'select' && (
          <div className="space-y-4">
            <h2 className="font-semibold text-slate-700 flex items-center gap-2"><Car className="w-4 h-4" /> Qual veículo você está devolvendo?</h2>
            {trips.map(t => (
              <label key={t.id} className={`flex items-start gap-3 border rounded-xl p-3 cursor-pointer transition-colors ${selectedTripId === t.id ? 'border-blue-600 bg-blue-50' : 'border-slate-200 bg-white'}`}>
                <input type="radio" value={t.id} {...register('trip_id')} className="accent-blue-700 mt-1" />
                <div>
                  <p className="font-semibold text-slate-800">{(t.vehicle as any)?.plate} — {(t.vehicle as any)?.brand} {(t.vehicle as any)?.model}</p>
                  <p className="text-sm text-slate-500">{(t.collaborator as any)?.name}</p>
            ...
            {errors.trip_id && <p className="text-red-500 text-sm">{errors.trip_id.message}</p>}
        ...
        {step === 'km' && (
          <div className="space-y-4">
            <h2 className="font-semibold text-slate-700 flex items-center gap-2"><Gauge className="w-4 h-4" /> Quilometragem de chegada</h2>
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 mb-1 block">KM atual do veículo <span className="text-red-500">*</span></label>
              <input type="number" {...register('km_arrival')} placeholder="Ex: 45590" className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-lg font-mono" />
              {errors.km_arrival && <p className="text-red-500 text-xs mt-1">{errors.km_arrival.message}</p>}
            </div>

            <div>
              <label className="text-sm font-medium text-slate-700 mb-1 block">Observações da chegada</label>
              <textarea {...register('notes_arrival')} rows={3} placeholder="Alguma ocorrência, dano ou observação..." className="w-full border border-slate-300 rounded-lg px-3 py-2.5 resize-none" />
            </div>

            <div className="flex gap-3">
              <button type="button" onClick={() => setStep('select')} className="flex-1 border border-slate-300 py-3 rounded-xl font-medium text-slate-700">Voltar</button>
        ...
        {step === 'photos' && (
          <div className="space-y-5">
            <h2 className="font-semibold text-slate-700">Fotos de devolução</h2>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setStep('km')} className="flex-1 border border-slate-300 py-3 rounded-xl font-medium text-slate-700">Voltar</button>
              <button
                type="button"
                onClick={() => {
        ...
        {step === 'confirm' && (
          <div className="space-y-4">
            <h2 className="font-semibold text-slate-700">Confirmar devolução</h2>
            ...
            <p className="text-xs text-slate-500 text-center">Ao confirmar, o veículo voltará para <strong>disponível</strong>.</p>
            <div className="flex gap-3">
    ...
    <div className="flex items-center gap-1 mt-3">
      {steps.map((s, i) => (
        <div key={s} className="flex items-center gap-1">
          <div className={`w-6 h-6 rounded-full text-xs flex items-center justify-center font-medium transition-colors ${i <= idx ? 'bg-blue-700 text-white' : 'bg-slate-200 text-slate-500'}`}>
            {i + 1}
          </div>
          {i < steps.length - 1 && <div className={`h-0.5 w-6 ${i < idx ? 'bg-blue-700' : 'bg-slate-200'}`} />}
        </div>
      ))}
      <span className="ml-2 text-xs text-slate-500">{labels[idx]}</span>
    </div>
  )
}
